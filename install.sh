...
create_launcher() {
    ...
    # Quoted heredoc: nothing in the launcher body is interpolated at generation
    # time, so a value containing $(...) or backticks cannot be baked into the
    # generated script and executed on every launch. The two real generation-time
    # values are written separately, and the install dir goes through %q so it
    # lands as a literal.
    {
        printf '%s\n' '#!/bin/bash' '# Claude Desktop for Linux -- launcher'
